...
type BookingDetails = TicketBookingDetails
...
export default function FindTicketPage() {
  ...
  const fetchBookingByReference = async (reference: string) => {
    ...
    try {
      ...
      if (response.ok && Array.isArray(data) && data.length > 0) {
        const normalizedBooking = normalizeTicketBooking(data[0])
        setBooking(normalizedBooking)
        if (window.location.hash === '#download') {
          setTimeout(() => {
            void downloadStructuredTicket(normalizedBooking)
          }, 100)
        }
      } else {
  ...
  const downloadTicket = async (bookingData: BookingDetails | null = booking) => {
    ...
    await downloadStructuredTicket(bookingData)
  }
